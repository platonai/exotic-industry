...
import ai.platon.scent.ml.BasicNodeEncoder
...
//                " -cellType PLAIN_TEXT"
    ...
    private val exportPath = Paths.get("/tmp/dataset-dom-20230108.csv")
    private val modelPath = Paths.get("/tmp/dom_decision_tree.pmml")
    private val predictResultPath = Paths.get("/tmp/dom-predict-result.csv")
    ...
    fun encodeElements() {
        ...
        IntRange(0, numChunks - 1).forEach { ident ->
            ...
            annotationTasks.take(50000)
                ...
                .forEach { document ->
                    val encoder = BasicNodeEncoder(nGram = options.nGram)
    ...
    private fun prepareAnnotationTasks(ident: Int) {
        ...
        LoadOptionDefaults.browser = BrowserType.PULSAR_CHROME

        val baseDir = AppPaths.DOC_EXPORT_DIR.resolve("annotated")
